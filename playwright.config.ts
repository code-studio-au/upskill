...
const trustedLocalTlsSpkiPin = (() => {
  ...
  if (!localTlsSpkiPin || !/^[A-Za-z0-9+/]{43}=$/u.test(localTlsSpkiPin))
    throw new Error(
      "PLAYWRIGHT_TLS_SPKI_PIN must identify the generated local TLS certificate",
    );
  return localTlsSpkiPin;
})();
...
const chromiumMobile = {
  ...devices["Pixel 7"],
  ...(trustedLocalTlsSpkiPin
    ? {
        launchOptions: {
          args: [
            `--ignore-certificate-errors-spki-list=${trustedLocalTlsSpkiPin}`,
          ],
        },
      }
    : {}),
