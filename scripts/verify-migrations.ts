...
try {
  ...
  const expectedTables = [
    ...
    "event_virtual_attendance_reconciliation",
    "event_virtual_attendance_decision",
    ...
    constraint_name: string;
      ...
      where table_schema = 'public'
        and constraint_name in (
          ...
          'contact_verification_channel_ck',
          'contact_verification_reference_ck',
          'contact_verification_digest_ck',
          'contact_verification_attempts_ck',
          'contact_verification_expiry_ck',
          'contact_verification_consumed_ck',
          'contact_verification_sms_capture_phone_ck',
          'contact_verification_purpose_ck',
          'contact_verification_context_ck'
        )`.execute(db);
  assert.equal(
    contactVerificationConstraints.rows.length,
    12,
    "Email and SMS verification state and challenges must be constrained",
  );
    ...
    constraint_name: string;
      ...
      where table_schema = 'public'
        and constraint_name in (
          ...
          'event_virtual_connection_interval_source_ck',
          'event_virtual_attendance_reconciliation_state_ck',
          'event_virtual_attendance_decision_uq',
