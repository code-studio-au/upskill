...
The repository now implements the governed authoring and versioning foundation
used by this decision:

- separate System and Offering Email catalogues;
- a seeded, code-owned Account Setup System Email contract;
- one editable draft per stable Email Design and immutable published versions;
- typed allowlisted variables, required-variable validation and fixture preview;
- publish, active-version rollback and exact notification-version pinning;
- persisted rendered subject, text and safely generated HTML delivery snapshots;
- Platform Administrator authoring boundaries and durable design audit actions;
- Mailgun/local-capture delivery of both text and generated safe HTML;
- Course Version and Event Template Version communication plans referencing an
...
Authoring is intentionally plain text in this phase. HTML is generated by
escaping all rendered content and applying only code-owned paragraph/line-break
...
Course and Event communications previously existed conceptually as a separate
list from the Sections and activities they support. That made it difficult for an
