...
export type LearnerEventRegistrationResult =
...
export async function registerLearnerForEventInTransaction(
  transaction: Transaction<Database>,
  eventOccurrenceId: string,
  eventOccurrenceRegionId: string | null,
  user: AuthenticatedUser,
): Promise<LearnerEventRegistrationResult> {
  const questionnaireRequirement = await transaction
    ...
    .select("event_template_version.registrationSurveyVersionId")
    .where("event_occurrence.id", "=", eventOccurrenceId)
    .executeTakeFirst();
  if (!questionnaireRequirement) return { status: "unavailable" } as const;
  if (questionnaireRequirement.registrationSurveyVersionId) {
  ...
  let eligibilitySource: "unrestricted" | "verified_domain" = "unrestricted";
  ...
  if (automaticallySelected && occurrence.confirmedCount >= occurrence.capacity)
    return { status: "unavailable" } as const;

  const registrationId = `event_registration_${randomUUID()}`;
  const registrationStatus = automaticallySelected ? "selected" : "submitted";
  await transaction
  ...
  if (automaticallySelected) {
    ...
    await transaction
      ...
      .values({
        ...
        nameSnapshot: user.name,
        emailSnapshot: user.email,
        detailsSubmittedAt: questionnaireRequirement.registrationSurveyVersionId
          ? now
          : null,
...
export async function registerLearnerForEvent(
  ...
  return await getDatabase()
    ...
    .execute(async (transaction) =>
      registerLearnerForEventInTransaction(
        transaction,
        eventOccurrenceId,
        eventOccurrenceRegionId,
        user,
