...
export async function up(db: Kysely<unknown>): Promise<void> {
  await sql`alter table learning_activity
    ...
    add column "surveyPosition" integer`.execute(db);
  await sql`update learning_activity as activity
    set "surveyType" = case
      when activity."surveyUsage" = 'onboarding' then 'system'
      when exists (
        select 1
          from learning_activity_version as version
          join course_version_item as item
            on item."learningActivityVersionId" = version.id
         where version."activityId" = activity.id
      ) and exists (
        select 1
          from learning_activity_version as version
          join event_template_version_item as item
            on item."learningActivityVersionId" = version.id
         where version."activityId" = activity.id
      ) then 'shared'
      when exists (
    ...
    where activity.kind = 'survey'`.execute(db);
  await sql`with positioned as (
      select id,
             row_number() over (
               partition by "surveyType"
               order by title, id
             ) - 1 as position
    ...
    update learning_activity
       set "surveyPosition" = positioned.position
      from positioned
     where positioned.id = learning_activity.id`.execute(db);
  await sql`alter table learning_activity
  ...
  await sql`alter table learning_activity
    add constraint learning_activity_survey_type_position_uq
    unique ("surveyType", "surveyPosition")
    deferrable initially deferred`.execute(db);
  await sql`create index learning_activity_survey_catalogue_idx
    on learning_activity ("surveyType", "surveyPosition", id)
    where kind = 'survey'`.execute(db);
...
export async function down(db: Kysely<unknown>): Promise<void> {
  await sql`drop index learning_activity_survey_catalogue_idx`.execute(db);
  await sql`alter table learning_activity
    drop constraint learning_activity_survey_type_position_uq,
    drop constraint learning_activity_survey_catalogue_ck,
    drop column "surveyPosition",
    drop column "surveyType"`.execute(db);
  // Durable audit rows are append-only, so the expanded action constraint must
  // remain valid if survey catalogue ordering is rolled back.
}
