...
export async function up(db: Kysely<unknown>): Promise<void> {
  ...
  await sql`alter table onboarding_sms_verification_capture
    drop constraint onboarding_sms_verification_capture_phone_ck,
    add constraint onboarding_sms_verification_capture_phone_ck
      check ("recipientPhone" ~ '^[+][1-9][0-9]{7,14}$')`.execute(db);
}
