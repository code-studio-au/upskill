...
const previousAuditActions = sql.raw(`
  ...
  'event_registration.withdrawn', 'event_registration.region_reassigned', 'event_region_review.locked',
...
export async function up(db: Kysely<unknown>): Promise<void> {
  await sql`alter table event_registration
    add column "regionMismatchAcknowledgedProfileRegionId" text references coordination_region(id) on delete restrict,
    add column "regionMismatchAcknowledgedAt" timestamptz,
    add column "regionMismatchAcknowledgedByUserId" text references "user"(id) on delete set null,
    add column "regionalReviewWaivedAt" timestamptz,
    add column "regionalReviewWaivedByUserId" text references "user"(id) on delete set null,
    add constraint event_registration_region_mismatch_ack_ck check (
      ("regionMismatchAcknowledgedAt" is null and "regionMismatchAcknowledgedByUserId" is null)
      or "regionMismatchAcknowledgedAt" is not null
    ),
    add constraint event_registration_regional_review_waiver_ck check (
      ("regionalReviewWaivedAt" is null and "regionalReviewWaivedByUserId" is null)
      or "regionalReviewWaivedAt" is not null
    )`.execute(db);
...
export async function down(db: Kysely<unknown>): Promise<void> {
  ...
  await sql`alter table event_registration
    drop constraint event_registration_regional_review_waiver_ck,
    drop constraint event_registration_region_mismatch_ack_ck,
    drop column "regionalReviewWaivedByUserId",
    drop column "regionalReviewWaivedAt",
    drop column "regionMismatchAcknowledgedByUserId",
    drop column "regionMismatchAcknowledgedAt",
    drop column "regionMismatchAcknowledgedProfileRegionId"`.execute(db);
}
