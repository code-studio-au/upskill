...
export async function up(db: Kysely<unknown>): Promise<void> {
  ...
  await db.schema
    .alterTable("access_grant")
    .addCheckConstraint(
      "access_grant_label_length_ck",
      sql`label is null or char_length(btrim(label)) between 2 and 120`,
    )
    .execute();
  await db.schema
    .createIndex("access_grant_admin_lookup_idx")
    .on("access_grant")
    .columns(["revokedAt", "createdAt"])
    .execute();
...
export async function down(db: Kysely<unknown>): Promise<void> {
  ...
  await db.schema
    .dropIndex("access_grant_admin_lookup_idx")
    .ifExists()
    .execute();
  await db.schema
    .alterTable("access_grant")
    .dropConstraint("access_grant_label_length_ck")
    .execute();
  await db.schema
    .alterTable("access_grant")
    .dropColumn("revokedByUserId")
    .dropColumn("revokedAt")
    .dropColumn("createdByUserId")
    .dropColumn("label")
    .execute();
}
