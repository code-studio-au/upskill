...
export async function up(db: Kysely<unknown>): Promise<void> {
  await sql`alter table event_registration_transition
    add column "fromEventOccurrenceRegionId" text references event_occurrence_region(id),
    add column "toEventOccurrenceRegionId" text references event_occurrence_region(id)`.execute(
    db,
  );
...
export async function down(db: Kysely<unknown>): Promise<void> {
  ...
  await sql`alter table event_registration_transition
    drop column "toEventOccurrenceRegionId",
    drop column "fromEventOccurrenceRegionId"`.execute(db);
}
