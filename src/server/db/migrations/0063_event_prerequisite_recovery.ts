...
export async function up(db: Kysely<unknown>): Promise<void> {
  await sql`create table event_prerequisite_recovery_challenge (
    id text primary key,
    reference text not null unique,
    "eventSurveyAccessId" text not null references event_survey_access(id) on delete cascade,
    "eventParticipationId" text not null references event_participation(id) on delete cascade,
    ...
    constraint event_prerequisite_recovery_reference_ck check (reference ~ '^[A-Za-z0-9_-]{32}$'),
    constraint event_prerequisite_recovery_digest_ck check (
      "identifierDigest" ~ '^[A-Za-z0-9_-]{43}$'
      and "requestFingerprint" ~ '^[A-Za-z0-9_-]{43}$'
      and "codeDigest" ~ '^[A-Za-z0-9_-]{43}$'
    ),
    constraint event_prerequisite_recovery_attempts_ck check (attempts between 0 and 5),
    constraint event_prerequisite_recovery_expiry_ck check ("expiresAt" > "createdAt"),
    constraint event_prerequisite_recovery_consumed_ck check ("consumedAt" is null or "consumedAt" >= "createdAt")
  )`.execute(db);
  await sql`create index event_prerequisite_recovery_rate_idx
    on event_prerequisite_recovery_challenge ("eventSurveyAccessId", "identifierDigest", "requestFingerprint", "createdAt" desc)`.execute(
    db,
  );

  await sql`create table event_prerequisite_task_session (
    id text primary key,
    "challengeId" text not null unique references event_prerequisite_recovery_challenge(id) on delete cascade,
    ...
    "createdAt" timestamptz not null default now(),
    constraint event_prerequisite_task_token_ck check ("tokenDigest" ~ '^[A-Za-z0-9_-]{43}$'),
    constraint event_prerequisite_task_expiry_ck check ("expiresAt" > "createdAt"),
    constraint event_prerequisite_task_state_ck check (not ("completedAt" is not null and "revokedAt" is not null))
  )`.execute(db);
  await sql`create index event_prerequisite_task_expiry_idx
    on event_prerequisite_task_session ("expiresAt")
    where "completedAt" is null and "revokedAt" is null`.execute(db);

  await sql`create table event_prerequisite_email_capture (
...
export async function down(db: Kysely<unknown>): Promise<void> {
  await sql`drop table event_prerequisite_email_capture`.execute(db);
  await sql`drop table event_prerequisite_task_session`.execute(db);
  await sql`drop table event_prerequisite_recovery_challenge`.execute(db);
  // Durable audit rows are append-only, so the expanded action constraint must
  // remain valid if the recovery tables are rolled back.
}
