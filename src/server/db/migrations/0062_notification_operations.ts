...
export async function up(db: Kysely<unknown>): Promise<void> {
  await replaceAuditConstraint(db, [
    ...previousAuditActions,
    "notification.delivery_requeued",
  ]);
  await sql`create index notification_operations_idx
    on notification (status, "createdAt" desc, id desc)`.execute(db);
}

export async function down(db: Kysely<unknown>): Promise<void> {
  await sql`drop index notification_operations_idx`.execute(db);
  // Durable audit rows are append-only, so the expanded action constraint must
  // remain valid even if the operational read-model index is rolled back.
}
