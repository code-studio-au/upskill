...
import classes from "./courses.$slug_.bulk-order.module.css";
...
function BulkOrderPage() {
  ...
  const minimumQuantity = course.bulkPricing.tiers[0]?.minimumQuantity ?? 2;
  const form = useForm({
    defaultValues: {
      slug: course.slug,
      organizationName: "",
    ...
    onSubmit: async ({ value }) => {
      ...
      try {
        const result = await startBulkOrderCheckout({ data: value });
        ...
        if (result.reason === "unauthenticated") {
          window.location.assign(
            `/login?redirect=${encodeURIComponent(`/courses/${course.slug}/bulk-order`)}`,
          );
          return;
        }
        setCheckoutError(
  ...
  const unitPrice = unitPriceFor(course.bulkPricing.tiers, values.quantity);
    ...
    <Container size="lg" className={classes.page}>
      <Stack gap="xl" className={classes.pageStack}>
        <header className={classes.header}>
          ...
          >
            <span aria-hidden="true">&larr;</span> Back to course
          </Link>
              ...
              <Stack gap="xl">
                  ...
                  <MantineNativeSelect
                    ...
                    data={[
                      { value: "shared_code", label: "One shared code" },
                      {
                        ...
                        label: "One unique code per seat",
                      },
                    ]}
                    onChange={(event) => {
                      form.setFieldValue(
                ...
                <section aria-labelledby="pricing-tiers-heading">
                  ...
                  <div className={classes.tiers}>
                    {course.bulkPricing.tiers.map((tier) => {
                      ...
                      return (
                        ...
                        >
                          <Text fw={700}>{tier.minimumQuantity}+ seats</Text>
                          <Text
                            fw={700}
